...
class ECGNet_V3(nn.Module):
    def __init__(self, input_channel=1, num_classes=34):
        # 卷积核大小
        sizes = [3, 5, 7]
        self.sizes = sizes
        # 叠加block个数, conv2d在前，conv1d在后(conv1d叠加4次)
        layers = [3, 1]
           

        super(ECGNet_V3, self).__init__()
        ...
        for i, size in enumerate(sizes):
            ...
            self.layers2.add_module('layer2_1', self._make_layer1d(BasicBlock1d, 256, layers[1], stride=2, size=size))
            self.layers2.add_module('layer2_2', self._make_layer1d(BasicBlock1d, 256, layers[1], stride=2, size=size))
            self.layers2.add_module('layer2_3', self._make_layer1d(BasicBlock1d, 256, layers[1], stride=2, size=size))
            self.layers2.add_module('layer2_4', self._make_layer1d(BasicBlock1d, 256, layers[1], stride=2, size=size))
