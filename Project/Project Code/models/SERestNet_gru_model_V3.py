...
__all__ = ['SERestNet_gru_sep8_model']
...
class GRU0(nn.Module):
    def __init__(self, input_size, hidden_size1, hidden_size2):  # , output_size
        ...
        self.seq_length = int(256 / input_size)
        self.n_layers = 2
    ...
    def forward(self, input):
        ...
        output = torch.cat((hidden[self.seq_length - 1], weight, avg_pool, max_pool), 1)
...
class RNN(nn.Module):
    ...
    def forward(self, x, age, gender):
        ...
        x = self.resnet(x)
        x = x.view(self.batch, 8, -1).transpose(0, 1).clone()
        self.total_len = x.size()[2]
...
def SERestNet_gru_sep8_model(pretrained=False, **kwargs):
